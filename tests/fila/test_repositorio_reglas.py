...
def test_noche_de_san_juan_no_pasa_al_dia_utc_siguiente():
    ...
    assert momento_utc.astimezone(ZONA_OPERATIVA).hour == 22
    assert _dia_operativo(momento_utc).isoformat() == "2026-03-10"
...
def test_turnos_fuera_de_ventana_se_rechazan_antes_de_ir_a_la_base(desplazamiento):
    fecha = datetime.now(ZONA_OPERATIVA).date() + timedelta(days=desplazamiento)
    with pytest.raises(ValueError, match="pasados|anticipación"):
