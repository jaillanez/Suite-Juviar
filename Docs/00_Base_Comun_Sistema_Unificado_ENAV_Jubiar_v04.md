...
| 1 | **RRHH / Higiene y Seguridad** | Legajos, EPP, licencias médicas, reclutamiento y capacitación | RRHH | `modulos/rrhh_epp`, `modulos/seleccion`, `modulos/capacitacion` |
...
| 7 | **Sitio público** | Recibe solicitudes comerciales desde la DMZ | Comercial | `apps/web`, `modulos/comercial` |
...
### 3.2 Terceros externos — un único registro
...
| Comprador exterior | Identificación fiscal + país; correo mientras Comercial lo califica |
| Postulante | DNI; correo mientras no lo tenga |
...
Un postulante vive dentro de Terceros con tipo `POSTULANTE`; no crea una cuarta lista de personas. El dueño del dato es RRHH. Si ingresa a trabajar, su registro se conserva y se vincula con el legajo creado en Nexus. El comprador exterior usa el tipo `COMPRADOR_EXTERIOR`: su identificación fiscal lleva código de país y puede quedar pendiente hasta que Comercial califique la solicitud.
...
**Etapa 1 — RRHH / EPP.** Es el primer módulo operativo porque ya cuenta con catálogo normativo y una base funcional. Necesita completar catálogo real de ítems, matriz aprobada, identidad y firma antes de reemplazar el papel.

**Etapa 2 — Selección.** Extiende Terceros con `POSTULANTE`, conserva los CV originales y agrega extracción y ranking explicables. Puede calibrarse con una muestra de 20 a 30 CV recibida por correo; no depende de la ruta de Chimbas.

**Etapa 3 — Capacitaciones.** Reutiliza el patrón de planilla, asistencia y el motor compartido de firma construido para EPP.

**Etapa 4 — Turnos y asistencia.** Queda detrás de las tres etapas anteriores porque está bloqueado por el diccionario de datos de Time y el protocolo mínimo de cambios de turno. No se escriben adaptadores sobre tablas supuestas.

**Etapa 5 — Cosecha y descarga de camiones.** Necesita: registro de terceros + maestro de sectores productivos. Se construye junto porque comparten los mismos actores. Ojo con la estacionalidad: hay que estar en producción **antes** de vendimia, no durante.

**Etapa 6 — Bot de consulta.** No se puede construir antes que la etapa 5: un bot sin datos que consultar no sirve de nada. Es una capa de lectura sobre la descarga de camiones.

**Etapa 7 — Declaración jurada.** Se ubica según qué DDJJ sea (ver §7). Si es un documento firmado, reutiliza el motor de firma de la etapa 0 y es de las cosas más rápidas de sumar.
...
### 6.5 Criterio de "verificado"

Una restricción de acceso o una validación no está verificada sólo porque su prueba termina en verde. Debe existir una prueba que intente la operación prohibida, casos explícitos de campo vacío y campo nulo cuando correspondan, y un control negativo que demuestre que la prueba deja de pasar si se quita la guarda que dice medir.

Los permisos de base se prueban conectando o ejecutando bajo el rol restringido contra la base donde fue aplicada la migración. Un `GRANT` o un `REVOKE` escrito en un archivo SQL no cuenta como verificado hasta que la operación permitida funciona y la prohibida falla por falta de privilegios.
